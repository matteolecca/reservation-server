...
export const updateUserPassword = async (password: string, id?: string): Promise<ResError> => await trycatch(UPDATE_PASSWORD, [password, id]);
export const checkEmailTaken = async (email: string): Promise<{ exist: boolean } | ResError> => await trycatch(CHECK_EMAIL, [email], true);
export const checkUsernameTaken = async (username: string): Promise<any | ResError> => await trycatch(CHECK_USERNAME, [username], true);
export const registerUserDevice = async (userId: string, token: string): Promise<ResError> => await trycatch(REGISTER_USER_DEVICE, [userId, token]);
export const updateRegisteredUserDevice = async (userId: string, token: string): Promise<ResError> => await trycatch(UPDATE_USER_REGISTERED_DEVICE, [userId, token], true);
